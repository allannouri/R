MSsqlExecute <- function(SQL = NULL, DATABASE= "XX", closeODBC = TRUE){
  # Remember to close the odbc connection
  createODBC <- function(DATABASE){
    library("RODBC")
    conn <- odbcDriverConnect(paste0("Driver={SQL Server}; Server=XX; Database=",
                                           DATABASE,
...
MSsqlSave <- function(DATA = NULL, DATABASE= "XX", TABLENAME = NULL, closeODBC = TRUE, OVERWRITE = FALSE){
...
UpdateLog <- function(ID, SUBID, TASK, FREQ, START = TRUE, END = FALSE){
  ...
  res <- MSsqlExecute(paste0("SELECT ID, SUB_ID, TASK FROM XX WHERE TASK = '", TASK, "'"))
  
  if(dim(res)[1] == 0){
    ID <-  MSsqlExecute("SELECT 1+MAX(ID) FROM XX")
    SUBID <- 1
    MSsqlExecute(paste0("INSERT INTO [XX VALUES (",paste0(ID),", ", SUBID, ", '",TASK,"', NULL, NULL,", 0, ", '", toupper(Sys.getenv("USERNAME")),"', '",FREQ,"' )"))
  print("Task is succesfully created")
  }
 
  res <- MSsqlExecute(paste0("SELECT ID, SUB_ID FROM XX WHERE ID = ", ID, " AND SUB_ID = ", SUBID))
  if(dim(res)[1] == 0){
    SUBID <-  MSsqlExecute(paste0("SELECT 1+MAX(SUB_ID) FROM XX WHERE ID = ", ID))
    MSsqlExecute(paste0("INSERT INTO XX VALUES (",paste0(ID),", ", SUBID, ", '",TASK,"', NULL, NULL,", 0, ", '", toupper(Sys.getenv("USERNAME")),"', '",FREQ,"' )"))
  print("SUB task is succesfully created")
  }
  
  if(isTRUE(START) && isFALSE(END)){
    MSsqlExecute(paste0("UPDATE XX SET START_TIME = '",
  ...
  if(isTRUE(END) && isFALSE(START)){
    EXEC_TIME <-  MSsqlExecute(paste0("SELECT START_TIME FROM XX WHERE ID = ", ID))
    EXEC_TIME <- difftime(Sys.time(),EXEC_TIME$START_TIME,units = "mins")
    MSsqlExecute(paste0("UPDATE XX SET END_TIME = '",
                        as.character(Sys.time()),"' , UPDATE_USER = '",toupper(Sys.getenv("USERNAME")),
...
AzureExecute <- function(SQL = NULL, DATABASE= NULL, closeODBC = FALSE){
  ...
  createODBC <- function(DATABASE){
    library("RODBC")
    conn <- odbcDriverConnect(paste0("Driver={ODBC Driver 17 for SQL Server}; Server=XX; Database=",
                                     DATABASE,
                                     "; Trusted_Connection=yes"))
  }
